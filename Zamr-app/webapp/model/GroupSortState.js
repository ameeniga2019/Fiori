sap.ui.define([
	...
	return BaseObject.extend("ZAMR-app.Zamr-app.model.GroupSortState", {
		 ...
		 * @alias ZAMR-app.Zamr-app.model.GroupSortState
		 */
		...
		/**
		 * Groups by Ntgew, or resets the grouping for the key "None"
		 *
		 * @param {string} sKey - the key of the field used for grouping
		 * @returns {sap.ui.model.Sorter[]} an array of sorters
		 */
		group: function (sKey) {
			...
			if (sKey === "Ntgew") {
				...
				this._oViewModel.setProperty("/sortBy", "Ntgew");

				aSorters.push(
					new Sorter("Ntgew", false,
						this._fnGroupFunction.bind(this))
				);
			} else if (sKey === "None") {
				// select the default sorting again
				this._oViewModel.setProperty("/sortBy", "Mtart");
			}
